...
Adafruit_NeoPixel_LJE::Adafruit_NeoPixel_LJE(uint8_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p)
...
Adafruit_NeoPixel_LJE::~Adafruit_NeoPixel_LJE() {
  pinMode(pin, INPUT);
}

void Adafruit_NeoPixel_LJE::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void Adafruit_NeoPixel_LJE::show(void) {
...
void Adafruit_NeoPixel_LJE::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void Adafruit_NeoPixel_LJE::setPixelColor(
...
void Adafruit_NeoPixel_LJE::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t Adafruit_NeoPixel_LJE::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t Adafruit_NeoPixel_LJE::getPixelColor(uint16_t n) const {
  if(n >= numLEDs) {
...
const uint8_t *Adafruit_NeoPixel_LJE::getPixels(void) const {
  return pixels;
}

uint16_t Adafruit_NeoPixel_LJE::numPixels(void) const {
  return numLEDs;
}
...
void Adafruit_NeoPixel_LJE::setBrightness(uint8_t b) {
...
uint8_t Adafruit_NeoPixel_LJE::getBrightness(void) const {
  return brightness - 1;
}

void Adafruit_NeoPixel_LJE::clear() {
